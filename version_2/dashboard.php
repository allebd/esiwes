<?php 
    ...
    $getadmins = "SELECT * FROM stafflist INNER JOIN staffrole ON stafflist.staffId = staffrole.staffNum INNER JOIN roles ON staffrole.staffRoleNo = roles.roleId WHERE staffdelete = '' AND staffId NOT IN ('$staffno')";
    $theadmins = $connection->query($getadmins);

    $getroles = "SELECT * FROM roles";
    $theroles = $connection->query($getroles);

    $getstaff = "SELECT * FROM stafflist WHERE staffId NOT IN ('$staffno')";
    $thestaff = $connection->query($getstaff);
								...
								<fieldset>
									<legend>New Supervisor</legend>	
										...
										<select name='staff' required>
											...
											<?php while($staffrow = $thestaff->fetch_assoc()){ ?>
											<option value='<?php echo $staffrow['staffId'];?>'><?php echo ucwords($staffrow['sname']);?> <?php echo ucwords($staffrow['fname']);?> <?php echo ucwords($staffrow['mname']);?></option>
											<?php } ?>
										</select>
										<label>
											Role:
